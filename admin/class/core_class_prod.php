...
class Core{
    ...
    public function get_data_pos(){
        ...
        $sql = $this->con->prepare("SELECT t2.id_loc, t2.lat, t2.lng, t2.sonido, t3.estados, t2.t_retiro, t2.t_despacho FROM fw_usuarios t1, locales t2, giros t3 WHERE t1.id_user=? AND t1.cookie_code=? AND t1.id_loc=t2.id_loc AND t2.cookie_code=? AND t2.cookie_ip=? AND t2.id_gir=t3.id_gir AND t1.eliminado=? AND t2.eliminado=? AND t3.eliminado=?");
    ...
    public function set_web_pedido(){
        ...
        $sql = $this->con->prepare("SELECT * FROM fw_usuarios t1, locales t2 WHERE t1.id_user=? AND t1.cookie_code=? AND t1.id_loc=t2.id_loc AND t2.cookie_code=?");
        $sql->bind_param("iss", $id, $user_code, $local_code);
        $sql->execute();
    ...
    public function enviar_pedido(){
        ...
        */
        ...
        $sqlipa->bind_param("issiiissiiiiiiisiiiii", $num_ped, $pedido_code, $fecha_stgo, $despacho, $tipo, $pedido["id_loc"], json_encode($_POST['carro']), json_encode($promos), $verify_despacho, $pedido["pre_gengibre"], $pedido["pre_wasabi"], $pedido["pre_embarazadas"], $pedido["pre_palitos"], $pedido["pre_teriyaki"], $pedido["pre_soya"], $pedido["comentarios"], $pedido["costo"], $pedido["total"], $id_puser, $pdir_id, $id_gir);
