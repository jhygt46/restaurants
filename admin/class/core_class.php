...
class Core{
    ...
    public function get_giros_user(){
        ...
        if($this->admin == 1 && $this->id_user > 1){ $giros = $this->con->sql("SELECT t2.id_gir, t2.nombre, t2.dominio FROM fw_usuarios_giros_clientes t1, giros t2 WHERE t1.id_user='".$this->id_user."' AND t1.id_gir=t2.id_gir AND t2.eliminado='0' ORDER BY dns_letra"); }
    ...
    public function get_giros(){
        $giros = $this->con->sql("SELECT id_gir, dominio FROM giros WHERE eliminado='0' ORDER BY id_gir DESC");
        return $giros['resultado'];
    }
    ...
    public function get_dominios_sin_dns(){
        $aux = $this->con->sql("SELECT id_gir, dominio FROM giros WHERE dns_letra IS NULL");
    ...
    public function get_ssl_sol(){
        $aux = $this->con->sql("SELECT id_gir, dominio FROM giros WHERE solicitar_ssl='1'");
        return $aux['resultado'];
    }
    ...
    public function get_info_catalogo($id_cat){
        ...
        for($i=0; $i<$categorias['count']; $i++){
            ...
            if($categorias['resultado'][$i]['tipo'] == 0){
                ...
                $prods_sql = $this->con->sql("SELECT * FROM cat_pros t1, productos t2, productos_precio t3 WHERE t1.id_cae='".$categorias['resultado'][$i]['id_cae']."' AND t1.id_pro=t2.id_pro AND t1.id_pro=t3.id_pro AND t3.id_cat='".$id_cat."' ORDER BY t1.orders");
