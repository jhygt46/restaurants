                            ...
                            if($sqlses = $this->con->prepare("SELECT * FROM ses_mail WHERE correo=?")){
                                if($sqlses->bind_param("s", $correo)){
                                    if($sqlses->execute()){
                                        ...
                                        if($id_loc == 0){
                                            if($sqlloc = $this->con->prepare("INSERT INTO locales (telefono, whatsapp, nombre, correo_ses, direccion, lat, lng, code, fecha_creado, fecha_cocina, correo, id_cat, id_gir) VALUES (?, ?, ?, ?, ?, ?, now(), now(), ?, ?, ?)")){
