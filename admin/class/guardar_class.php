...
class Guardar{
    ...
    private function eliminar_gra_lista(){
        ...
        if(isset($this->id_gir) && is_numeric($this->id_gir) && $this->id_gir > 0){
            $id = explode("/", $_POST['id']);
            if($sql = $this->con->prepare("DELETE FROM set_graficos_id WHERE id_grf IN (SELECT id_grf FROM set_graficos t1, set_graficos_id t2 WHERE t1.id_gir=? AND t1.id_set=? AND t1.id_set=t2.id_set AND t2.id_grf=?)")){
