...
for($i=0; $i<count($polygons); $i++){
    ...
    for($j=0; $j<count($polygon); $j++){
        ...
        $polygon[$j]->{'lat'};
        $polygon[$j]->{'lng'};
        $poli[] = $polygon[$j]->{'lat'}." ".$polygon[$j]->{'lng'};
    ...
    if($is == "inside" || $is == "vertex"){
        if($polygons[$i]['precio'] < $precio){
            $info['op'] = 1;
            $info['id_loc'] = $polygons[$i]['id_loc'];
            $info['nombre'] = $polygons[$i]['nombre'];
            $info['precio'] = $polygons[$i]['precio'];
            $info['direccion'] = $polygons[$i]['direccion'];
        }
    }
